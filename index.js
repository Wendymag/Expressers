...
app.post("/register", body('email').isEmail().normalizeEmail(),body('password').isLength({min: 6}), (req, res, next) => {
    ...
    firestore
      ...
      .then(() => {
        res.status(200).json({success: true,
          message: 'Registered successful',userCreated: user});
      })
...
app.put('/employees/:id', body('email').isEmail().normalizeEmail(),body('password').isLength({min: 6}), (req, res, next)=>{
  const errors = validationResult(req);
